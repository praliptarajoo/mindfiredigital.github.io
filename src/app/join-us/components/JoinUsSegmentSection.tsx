  ...
  return (
    <section className={cn("mt-[6rem] max-w-6xl mx-auto px-6", className)}>
      <h3 className="tracking-widest text-3xl font-medium capitalize text-mindfire-text-black text-center">
        {title} <span className="text-mf-dark font-semibold tracking-widest">FOSS</span>
      </h3>
          ...
          ? data.map(({ title, description }, index) => {
              return (
                <div key={index} className="flex gap-4">
                  <div className="px-4 text-[2.6rem] leading-[3.4rem] text-mf-light-grey font-semibold">{index + 1}</div>
                  <div className="px-4">
